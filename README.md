...
Примеры из `Стычки_и_рефреймин_2 (Responses).csv` калибруют промпты шагов 5 и 7 под реальные случаи (в т.ч. паттерн самоиронии в СП-категории для АЖ-я). Лежат в `worker/fewshot.data.js` (без имён/фамилий, роли обозначены общо — "мама", "начальник" и т.п.), `worker/claude-proxy.js` импортирует из него. Куратированная выборка (~12 примеров), не весь CSV целиком — держит промпт компактным.

- Формат/шаблон — `worker/fewshot.data.example.js`.
