...
const MASK_SCHEMA = {
  ...
  properties: {
    ...
    roleCheck: { anyOf: [{ type: "string" }, { type: "null" }] },
  },
  required: ["mask", "inversion", "reasoning", "roleCheck"],
...
Дополнительная проверка согласованности роли (поле "roleCheck"): АЖ-другой означает, что ИМЕННО ФИГУРА требует моей подстройки под свою нарциссическую потребность (self-object) — фигура выставляет счёт, я чувствую вину/обязанность обслуживать её образ себя. Это НЕ то же самое, что фигура своим поведением причиняет пользователю реальный вред/боль — это другой конструкт (attachment threat / реальная задетость, не self-object провокация). Симметрично для АЖ-я: роль означает, что пользователь давит на другого, требуя подстройки под свою потребность — не то же самое, что пользователь описывает себя пострадавшей стороной от поведения другого.
Если из текста шагов 1-4 (зуб, фигуры, ядерная нагрузка) видно, что пользователь фактически описывает себя как пострадавшую/раненую сторону от поведения фигуры (тревога, боль, невозможность расслабиться из-за того, что фигура делает или чувствует), а не как сторону, обязанную обслуживать нарциссическую потребность фигуры (или наоборот, для АЖ-я) — это возможное несовпадение с выбранной ролью. В этом случае заполни "roleCheck" короткой (1-2 предложения) мягкой пометкой, предлагающей пользователю перепроверить выбор роли на шаге 4 — не утверждай, что роль неверна, просто дай сигнал задуматься. Если явного несовпадения нет — "roleCheck" строго null, не выдумывай сомнение туда, где его нет.
...
${userGuess ? `\nСобственная догадка пользователя о маске (учти, но не следуй слепо): ${userGuess}` : ""}${nuclearLoadLine(nuclearLoad)}`;
  ...
  return {
    ...
    roleCheck: parsed.roleCheck || null,
  };
}
